...
figure
subplot(2, 1, 1);
plot(x_plot,m_plot);hold on; plot(x_plot,h_plot);plot(x_plot,n_plot); xlabel('Time (ms)'); ylabel('H-H parameters');
legend('m', 'h', 'n')

subplot(2, 1, 2);
plot(x_plot,I_plot);
legend('I_{K}', 'I_{Na}', 'I_{lk}'); xlabel('Time (ms)'); ylabel('The membrane currents');
